...
defineFeature(feature, (test) => {
  ...
  test('User should see a list of suggestions when they search for a city.', ({ given, when, then }) => {
    ...
    let CitySearchDOM;
    when('user starts typing in the city in the textbox', async () => {
      const AppDOM = AppComponent.container.firstChild;
      const user = userEvent.setup();
      CitySearchDOM = AppDOM.querySelector('#city-search');
      const citySearchInput = within(CitySearchDOM).queryByRole('input');
  ...
  test('User can select a city from the suggested list.', ({ given, and, when, then }) => {
    ...
    given('user was typing “Berlin” in the city textbox', async () => {
      ...
      CitySearchDOM = AppDOM.querySelector('#city-search');
      citySearchInput = within(CitySearchDOM).queryByRole('input');
      await user.type(CitySearchDOM, "Berlin");
    });
    ...
    then('their city should be changed to that city (i.e., “Berlin, Germany”)', () => {
      expect(CitySearchDOM.value).toBe('Berlin, Germany');
    });
    ...
    and('the user should receive a list of upcoming events in that city', async () => {
      ...
      const berlinEvents = allEvents.filter(event => event.location === CitySearchDOM.value)
